...
internal class ReflectUtils {
	...
	static Bool paramTypesFitMethodSignature(Type[] params, Method method) {
		return method.params.all |funcParam, i->Bool| {
			if (i >= params.size)
				return funcParam.hasDefault
			if (!params[i].fits(funcParam.type))
				return false
			return true
		}		
	}

	static Bool paramTypesFitFuncSignature(Type[] params, Func func) {
		return func.params.all |funcParam, i->Bool| {
			if (i >= params.size)
				return funcParam.hasDefault
			if (!params[i].fits(funcParam.type))
				return false
